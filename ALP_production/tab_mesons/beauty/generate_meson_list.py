...
for nprod in [100000]:#[100000,300000,1000000]:
    for p_beam in [400]:#[120, 400, 800]:
        print("Prdouction for impact on fixed proton by proton with momentum",p_beam)
        pythia = pythia8.Pythia("-v", False)
        pythia_modifiers = ["HardQCD:hardbbbar = on", "Beams:idA = 2212","Beams:idB = 2212","Beams:eA = " + str(np.sqrt(p_beam**2+0.93827**2)), "Beams:eB = "+str(0.93827), "Beams:frameType = 2"]
        ...
        np.savetxt(f"./hsbbbar_{p_beam}GeV_{number_to_3sigfigs_str(nprod)}.dat", meson_list, header=file_info, fmt='%i %4f %4f %3f')
...
for p_beam in [400]:#70,120,400,800
    ...
    pythia_modifiers = ["PhaseSpace:pTHat3Min = 3","PhaseSpace:pTHat5Min = 0.5","MultipartonInteractions:ecmRef = 30","HardQCD:nQuarkNew = 5", "HardQCD:3parton = on","Beams:idA = 2212","Beams:idB = 2212","Beams:eA = " + str(np.sqrt(p_beam**2+0.93827**2)), "Beams:eB = "+str(0.93827), "Beams:frameType = 2"]
    ...
    pythia.stat()
    file_info  =  "Charmed meson momenta as generated for " + str(nprod) +" impining protons, with PYTHIA (version "+str(pythia.parm("Pythia:versionNumber"))+") and modifiers [" + ', '.join(pythia_modifiers)+']'
    ...
    np.savetxt(f"./3partonbbbar_{p_beam}GeV_{number_to_3sigfigs_str(nprod)}.dat", meson_list, header=file_info, fmt='%i %4f %4f %3f')
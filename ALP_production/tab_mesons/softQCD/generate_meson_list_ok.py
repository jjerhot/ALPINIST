...
for nprod in [100_000]:
    for p_beam in [800]:
        print("Prdouction for impact on fixed proton by proton with momentum",p_beam)
        pythia = pythia8.Pythia("-v", False)
        pythia_modifiers = ["SoftQCD:all = on","pdf:pHardSet = 2", "111:mayDecay = no","Beams:idA = 2212","Beams:idB = 2212","Beams:eA = " + str(p_beam), "Beams:frameType = 2"]
        for modifier in pythia_modifiers+["Next:numberShowEvent = 5"]:
            pythia.readString(modifier)

        pythia.init() # initialize
        meson_list = []
        for iEv in range(nprod):
            pythia.next()
            for isubEvent in range(pythia.event.size()):
                evt = pythia.event[isubEvent]
                if evt.idAbs() not in [111, 221, 331, 113, 213, 223, 333]: continue
                meson_list.append([iEv,isubEvent,evt.id(),evt.px(),evt.py(),evt.pz(),evt.status()])
        
        file_info  =  "Light meson momenta as generated for " + str(nprod) +" impining protons, with PYTHIA"+str(pythia.parm("Pythia:versionNumber"))+" and modifiers [" + ', '.join(pythia_modifiers)+']'
        np.savetxt(f"./softQCD_{number_to_3sigfigs_str(nprod)}evts_pp_{p_beam}GeV_8.3_pSet2_pom_ok.txt", meson_list, header=file_info, fmt='%i %i %i %4f %4f %3f %i')
